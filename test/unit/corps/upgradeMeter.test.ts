...
import { UPGRADE_METER_WINDOW, resetUpgradeMeterHeapMarker, tallyUpgradeAttempt } from "../../../src/corps/UpgradingCorp";
...
describe("tallyUpgradeAttempt (upgrade WORK-utilization meter)", () => {
  ...
  it("a FRESH HEAP (global reset - every deploy) rolls the window even mid-cadence", () => {
    // Measured t72744628: a Memory-persisted window straddling the 4b deploy
    // read workUtil 0.999 / X1 0.10 across the boundary while the post-deploy
    // truth was burn 18 of 57 standing WORK - the stale weighting cost the
    // whole acceptance read. A deploy's reset now forces each room's first
    // tally to open a fresh window; the histogram still rides through.
    const meter: any = {};
    tallyUpgradeAttempt(meter, "W43N23", 100, OK_RC);
    meter.W43N23.hist.windows = 3;
    resetUpgradeMeterHeapMarker(); // the deploy
    tallyUpgradeAttempt(meter, "W43N23", 150, DRY_RC); // well inside the old cadence
    expect(meter.W43N23, "post-deploy window starts clean").to.deep.include({ t0: 150, ticks: 1, fired: 0, dry: 1 });
    expect(meter.W43N23.hist.windows, "the histogram survives the reset roll").to.equal(3);
  });
});
