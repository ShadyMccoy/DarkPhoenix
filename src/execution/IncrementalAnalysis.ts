...
export function runIncrementalAnalysis(colony: Colony): boolean {
  // RE-ANALYSIS IS OPT-IN (2026-08-11 crash-loop incident, INVERTED
  // DEFAULT): with two owned rooms, the post-reset analysis RESTART
  // hard-killed the global (Memory unpersisted from t72933848, losses
  // windowTicks 6, blackbox ring ~1KB, twelve console/API flag writes never
  // landing, fleet 44 -> 3 over ~1,800t). A dead bot can execute NO console
  // flag, so the only workable kill switch is one the DEPLOY itself arms:
  // RE-analysis runs only when Memory.analysisGo === 1 is set from a
  // healthy console. Post-hold the colony stabilized in one global (13
  // creeps and climbing, 173 reset-free ticks at first read).
  //
  // Scope: the hold applies only when persisted NODES exist - a colony that
  // can run on its node store holds safely on stale territories. A true
  // cold start (zero nodes: fresh respawn, fresh test world) has no economy
  // without the analysis and always runs it - the incident's killer was the
  // restart at established-colony scale, never the cold start. Do not set
  // analysisGo until the batch step is instrumented (heap/CPU stamps).
  if ((Memory as { analysisGo?: number }).analysisGo !== 1 && colony.getNodes().length > 0) {
    incrementalState = null;
    return true;
  }
