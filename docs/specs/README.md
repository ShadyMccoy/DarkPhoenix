...
| 33 | [Wartime (build) economy](33-wartime-build-economy.md) | BACKLOG 2026-07-26 (owner idea) — a MODE while construction stands: relegate upgrading to the controller-floor sip, re-point the surplus-eater consumer doctrine (`sustainableConsumptionRate`) from controller to CONSTRUCTION, and size the haul fleet to the build absorb rate (bigger bodies), to finish projects ASAP. Directly enables the spec-31 remodel; guardrails = never starve income, controller floor inviolable, clean exit | P2 |
