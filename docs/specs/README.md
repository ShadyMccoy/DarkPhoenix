...
| 08 | [Inflection-point grid](08-inflection-grid.md) | ~130 cells; BOT LEVEL ratcheted (current reds, 2026-07-28 full runs: haul-t3-dedicated-resume-groundpile + plan-t4-link-haul-pricing — the bisected #143 master regressions; fid-t4-synthetic's batch-load-marginal refill SLA — spec 34 open item; exp-t5-founding timeout — active work. haul-t4-tender-bus-regime and plan-t5-remote-pipeline are GREEN again) | ongoing — the success metric |
...
| 34 | [Operation corps: the corp as economic interface](34-operation-corps.md) | **ACTIVE 2026-07-27** (owner thesis) — simple economic interface up (positions/rates/ALL-IN price), sophistication inside, faithfulness measured. LANDED: buffer/vector/crossover primitives, parked buffered builder (D1-D3b incl. unladen relocation), all-in construction price (D4), D6 cohort release at confirmed operation end (tender-rescue retired; want-dip = defund, never release), `builder-buffer-feed` cell green at 91-92% workUtil after two fixes it forced (parked burn-on-any-energy, operation-end release gate). OPEN: the vector-gait tension (measured both ways, mechanism undiagnosed — 1:1 carriers lift the cell to 98-100% but collapse the fid-t5 maze ramp), the minerCorp producer mirror (D5 second half), P4 ledger consistency | P1 |
