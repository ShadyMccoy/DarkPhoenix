...
export class RoomGeography {
   ...
   */
  analyzeRoom(room: Room): RegionNode[] {
    const distanceMatrix = RoomGeography.createDistanceTransform(room);
    const peaks = RoomGeography.findPeaks(distanceMatrix, room);
    const edges = this.createEdges(peaks, room);
    return RoomGeography.peaksToRegionNodes(room, peaks);
  }

    private peaksToRegionNodes(room: Room, peaks: Peak[]): RegionNode[] {
        const nodes: RegionNode[] = [];
        for (const peak of peaks) {
            const resources: RegionNode['resources'] = [];
            // Find sources, minerals, controller near the center
            const sources = room.find(FIND_SOURCES).filter(s => s.pos.getRangeTo(peak.center) <= 5);
            const minerals = room.find(FIND_MINERALS).filter(m => m.pos.getRangeTo(peak.center) <= 5);
            const controller = room.controller && room.controller.pos.getRangeTo(peak.center) <= 5 ? room.controller : null;

            sources.forEach(s => resources.push({ position: s.pos, type: 'source', id: s.id }));
            minerals.forEach(m => resources.push({ position: m.pos, type: 'mineral', id: m.id }));
            if (controller) resources.push({ position: controller.pos, type: 'controller', id: controller.id });

            nodes.push({
                id: RoomGeography.getNodeId(peak.center),
  ...
  private createEdges(peaks: Peak[], room: Room): Edge[] {
    ...
    const adjacentNodes = RoomGeography.getAdjacentRoomNodes(room);
    ...
    return RoomGeography.pruneEdges(edges, this.nodes);
  }
  ...
  private static parseRoomName(roomName: string): { x: number; y: number } {
    const [, wx, wy] = /^[WE](\d+)[NS](\d+)$/.exec(roomName) || [];
    const x = parseInt(wx);
    const y = parseInt(wy);
    return { x, y };
  }
  ...
  private static saveNode(node: RegionNode): void {
    ...
    Memory.nodeNetwork.nodes[node.id] = {
      pos: new RoomPosition(node.position.x, node.position.y, node.position.roomName),
      height: 0, // TODO: calculate height
      territory: node.territory.map(pos => new RoomPosition(pos.x, pos.y, pos.roomName)),
      resources: node.resources.map(resource => ({
        id: resource.id!,
        type: resource.type as "source" | "mineral" | "controller",
        pos: new RoomPosition(resource.position.x, resource.position.y, resource.position.roomName)
      }))
    };
  }
  ...
  private static saveEdges(edges: Edge[]): void {
    ...
    for (const edge of edges) {
      ...
      Memory.nodeNetwork.edges[edgeId] = {
        ...
        path: edge.path.map(pos => new RoomPosition(pos.x, pos.y, pos.roomName)),
        cost: edge.cost,
  ...
  private isValidPosition(x: number, y: number, terrain: RoomTerrain): boolean {
