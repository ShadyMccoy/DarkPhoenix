 ...
 * 58's storageAbsorbRate on the ullage side - but in different modules, with
...
describe("economy/bank - the pressure pair IN THE PLAN (empty -> full sweep)", () => {
  ...
  it("the contraction is SOURCE BY SOURCE in the taper, not a cliff (the spec 58 property, via the pair)", () => {
    ...
    // and demote. A cliff would drop all four at once (the pre-spec-58 shape).
