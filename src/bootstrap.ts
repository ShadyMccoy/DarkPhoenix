...
export function bootstrap(room: Room) {
    ...
    let numCreeps = room.find(FIND_MY_CREEPS).length;

    if (!spawn.spawning) {
        if (jacks.length <  (numCreeps > 2 ? 1 : 3)) {
            spawn.spawnCreep([WORK, CARRY, MOVE], spawn.name + Game.time, { memory: { role: "jack" } });
        }
    };

    forEach(jacks, (jack) => {
        if (jack.store.energy == jack.store.getCapacity()) {
                DeliverEnergyToSpawn(jack, spawn);
        } else {
            if (!pickupEnergyPile(jack)) {
                HarvestNearestEnergySource(jack);
            }
        }
    });
}
...
function HarvestNearestEnergySource(creep: Creep): boolean {
...
function BuildMinerContainer(creep: Creep) {
...
function pickupEnergyPile(creep: Creep): boolean {
...
function DeliverEnergyToSpawn(creep: Creep, spawn: StructureSpawn): number {
    creep.say('deliver');
...
function dismantleWalls(creep: Creep): void {
...
function getScaledBody(body: BodyPartConstant[], scale: number): BodyPartConstant[] {
