/**
 * sim-labs-emergent.ts — EMERGENT LOCAL-RULE, REACT-AWAY FLOW (spec 31).
 *
...
function run(args: Args): void {
  ...
  // allocator.
  const hunger = (c: string): number => BUFFER - stockOf(c) + (c === target ? 1 : 0);

  const react = (): void => {
    // PRODUCE: each held reactor refreshes its own compound, low tiers first so
    // fresh output is readable by the next tier within the same tick.
    const held = reactorIds.map((i) => labs[i]).filter((l) => l.mineral !== null && l.cooldown === 0);
    held.sort((x, y) => depthOf(x.mineral!) - depthOf(y.mineral!));
    for (const P of held) {
      const c = P.mineral!;
      if (c !== target && stockOf(c) >= BUFFER) continue; // satisfied -> idle (may drain & swing)
      if (P.amount + LAB_REACTION_AMOUNT > CAP) continue;
      fire(P, c);
    }
    // ADOPT: an empty reactor takes the hungriest compound it can currently make.
  ...
  const advanceTender = (): void => {
    ...
    if (tender.carrying) {
      ...
      if (tl) {
        ...
        tl.amount -= amt;
        if (tl.amount === 0) tl.mineral = null; // drained -> free to adopt again
  ...
  console.log(`  scheduler       : EMERGENT LOCAL-RULE FLOW (reactors follow make-or-adopt-hungriest; allocation is NOT assigned)`);
